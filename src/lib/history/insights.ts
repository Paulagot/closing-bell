...
function marketMove(issuer: IssuerHistoryAnalytics): HistoricalInsight {
  ...
  if (usable.length < 2) {
    return {
      ...
      body: "Closing Bell needs at least two stored observations containing both the Wall Street benchmark and executable $1K BUY price before it can separate the underlying stock move from wrapper divergence.",
      status: "waiting",
    };
  }
...
function maturity(input: Input): HistoricalInsight {
  ...
  if (input.issuer.observations < MIN_CONTEXT) {
    return {
      ...
      body: `Closing Bell has ${input.issuer.observations} stored observation${input.issuer.observations === 1 ? "" : "s"} for this wrapper. Treat the current range as an early read rather than a normal trading pattern.`,
      status: "waiting",
    };
  }
  ...
  if (hours < 24) {
    return {
      ...
      body: `There are ${input.issuer.observations} stored observations spanning about ${Math.max(1, Math.round(hours))} hours. This can show whether the current reading is unusual today, but it cannot yet answer a full 24-hour convergence question.`,
      status: "early",
    };
  }
  ...
  return {
    ...
    body: `Closing Bell has ${input.issuer.observations} stored observations across about ${Math.round(hours)} hours. Longer-lived patterns become more useful as more market sessions are collected.`,
...
function currentGap(issuer: IssuerHistoryAnalytics): HistoricalInsight {
  ...
  if (current === null) {
    return {
      ...
      body: "The latest stored observation does not contain a usable $1K executable BUY gap.",
      status: "waiting",
    };
  }
  ...
  if (issuer.observations < MIN_CONTEXT || percentile === null) {
    return {
      ...
      body: `The latest stored Wall Street benchmark is ${money(latestPoint(issuer)?.benchmarkPrice)} and the $1K executable BUY is ${money(latestPoint(issuer)?.buyPrice)}. There are not enough stored observations yet to say how unusual this reading is.`,
      status: "early",
    };
  }
  ...
  if (percentile >= UNUSUAL_PERCENTILE) {
    return {
      ...
      body: `The current $1K executable BUY is ${money(latestPoint(issuer)?.buyPrice)} versus a stored Wall Street benchmark of ${money(latestPoint(issuer)?.benchmarkPrice)}, a gap of ${pct(current)}. Its magnitude is larger than about ${percentile.toFixed(0)}% of observations collected for this wrapper.`,
  ...
  return {
    ...
    body: `The current $1K executable BUY is ${money(latestPoint(issuer)?.buyPrice)} versus a stored Wall Street benchmark of ${money(latestPoint(issuer)?.benchmarkPrice)}, a gap of ${pct(current)}. Its magnitude ranks around the ${percentile.toFixed(0)}th percentile of observations collected so far.`,
    status: issuer.observations >= 20 ? "available" : "early",
  };
}
...
function distanceFromMedian(issuer: IssuerHistoryAnalytics): HistoricalInsight {
  ...
  if (current === null || med === null) {
    return {
      ...
      body: "Closing Bell needs more usable stored BUY observations before it can describe the distance from the recent median.",
      status: "waiting",
    };
  }
...
function executionReality(issuer: IssuerHistoryAnalytics): HistoricalInsight {
  ...
  if (!point || point.indicativeGapPct === null || point.buyGapPct === null) {
    return {
      ...
      body: "The latest stored point needs both an indicative gap and an executable BUY gap.",
      status: "waiting",
    };
  }
  ...
  if (indicative < 0 && executable >= 0) {
    return {
      ...
      body: `The indicative wrapper price was ${money(point.indicativePrice)} (${pct(indicative)} versus Wall Street), but the stored $1K executable BUY was ${money(point.buyPrice)} (${pct(executable)}). The headline discount did not survive execution.`,
      status: "available",
    };
  }
  ...
  if (indicative < 0 && executable < 0) {
    return {
      ...
      body: `The indicative wrapper price was ${money(point.indicativePrice)} (${pct(indicative)}) and the stored $1K executable BUY was ${money(point.buyPrice)} (${pct(executable)}). Execution changed the apparent discount by ${Math.abs(executable - indicative).toFixed(2)} percentage points.`,
      status: "available",
    };
  }
  ...
  return {
    ...
    body: `The indicative wrapper price was ${money(point.indicativePrice)} (${pct(indicative)}) and the stored $1K executable BUY was ${money(point.buyPrice)} (${pct(executable)}). Closing Bell uses the executable figure when describing what a user could actually trade.`,
    status: "available",
  };
}
...
function friction(issuer: IssuerHistoryAnalytics): HistoricalInsight {
  ...
  return {
    ...
    body:
      percentile === null
        ? `The latest stored $1K BUY is ${money(latestPoint(issuer)?.buyPrice)} and SELL is ${money(latestPoint(issuer)?.sellPrice)}. This is the approximate relative move required for the current SELL price to equal the current BUY price. More history is needed to judge whether this is unusually high or low.`
        : `The latest stored $1K BUY is ${money(latestPoint(issuer)?.buyPrice)} and SELL is ${money(latestPoint(issuer)?.sellPrice)}. This break-even reading is around the ${percentile.toFixed(0)}th percentile of recorded observations.`,
...
function convergence(issuer: IssuerHistoryAnalytics): HistoricalInsight {
  ...
  if (events === 0) {
    return {
      ...
      body: "A qualifying episode starts when the stored $1K executable BUY moves at least 0.75% from Wall Street. It needs a full 24-hour outcome window before Closing Bell can classify whether it returned inside ±0.50%.",
      status: "waiting",
    };
  }
...
function usOpen(issuer: IssuerHistoryAnalytics): HistoricalInsight {
  if (issuer.openObservations === 0) {
    return {
      ...
      body: "Closing Bell needs a stored observation before 9:30 ET and another around 10:00–10:30 ET on the same day before it can compare whether the wrapper gap narrowed after Wall Street opened.",
      status: "waiting",
    };
  }
  ...
  return {
    ...
    body: `${issuer.openObservations} complete US-open comparison${issuer.openObservations === 1 ? "" : "s"} ${issuer.openObservations === 1 ? "is" : "are"} available. ${issuer.openObservations < MIN_PATTERN ? "That is useful context, but still too few sessions to call a pattern." : "This compares the last stored gap before 9:30 ET with a stored gap during the first hour after the open."}`,
...
function reliability(issuer: IssuerHistoryAnalytics): HistoricalInsight {
  ...
  if (a === null) {
    return {
      ...
      body: "There are not enough stored observations to estimate matched executable BUY-and-SELL availability.",
      status: "waiting",
    };
  }
  ...
  return {
    ...
    body: `Both executable BUY and SELL quotes were available in ${a.toFixed(0)}% of stored observations for this wrapper.`,
    status: issuer.observations >= MIN_CONTEXT ? "available" : "early",
  };
}
